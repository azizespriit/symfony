...
class ProduitController extends AbstractController
{
    ...
    public function catalogue(EntityManagerInterface $entityManager, Request $request, PaginatorInterface $paginator): Response
    {
        ...
        $pagination = $paginator->paginate(
            ...
            $request->query->getInt('page', 1), // Le numéro de page par défaut est 1
        ...
        if ($form->isSubmitted()) {
            ...
            if (!$form->isValid()) {
                ...
                if (count($errors) > 0) {
                    ...
                    $this->addFlash('error', 'Le formulaire n\'est pas valide mais aucune erreur spécifique n\'a été détectée.');
                }
            }
            
            if ($form->isValid()) {
                $photoFile = $form->get('photoFile')->getData();
                $generatedImageFilename = $request->request->get('generated_image_filename');
                
                // Si une image a été uploadée via le formulaire
                if ($photoFile) {
                    $originalFilename = pathinfo($photoFile->getClientOriginalName(), PATHINFO_FILENAME);
                    $safeFilename = $slugger->slug($originalFilename);
                    $newFilename = $safeFilename.'-'.uniqid().'.'.$photoFile->guessExtension();
                    ...
                    try {
                        ...
                        $uploadDir = $this->getParameter('produit_images_directory');
                ...
                // Si une image a été générée par Eden AI
                elseif ($generatedImageFilename) {
                    $produit->setPhoto($generatedImageFilename);
                }
