...
class PanierController extends AbstractController
{
    ...
    public function addProduit(Request $request, EntityManagerInterface $entityManager, int $id, SessionInterface $session): Response
    {
        ...
        // Skip the panier selection if there's only one panier
        if (count($paniers) === 1) {
            return $this->redirectToRoute('panier_confirm_add');
        }
        
        return $this->render('panier/front/panieradd.twig', [
            'produit' => $produit,
            'panier' => $panier,
            'paniers' => $paniers,
            'quantite' => $request->query->get('quantite', 1)
    ...
    public function confirmAdd(Request $request, EntityManagerInterface $entityManager, SessionInterface $session): Response
    {
        ...
        $panierId = $request->query->get('panier_id') ?: $session->get('panier_id');
        ...
        // Save the selected cart ID to the session
    ...
    {
        // Get all available paniers for the dropdown
        $allPaniers = $entityManager->getRepository(Panier::class)->findAll();
        
        // Get panier ID from request or from session
        $requestPanierId = $request->query->get('panier_id');
        $panierId = $requestPanierId ?: $session->get('panier_id');
        $panier = null;
        $panierProduits = [];
        
        // If there are no paniers at all, we don't need to do anything special here
        // The template will show the appropriate message and buttons
        
        if ($panierId) {
            $panier = $entityManager->getRepository(Panier::class)->find($panierId);
            
            if ($panier) {
                // Update session with the selected panier
                $session->set('panier_id', $panierId);
    ...
    #[Route('/new-and-add', name: 'panier_new_and_add')]
    public function newAndAdd(EntityManagerInterface $entityManager, SessionInterface $session): Response
    {
        $pendingProduct = $session->get('pending_product');
        ...
        $panier = new Panier();
        $entityManager->persist($panier);
        $entityManager->flush(); // Flush to get the cart ID
        
        // Save the new cart ID to the session
        $session->set('panier_id', $panier->getId());
        
        // Insert new cart item using direct SQL
        $conn = $entityManager->getConnection();
        $sql = 'INSERT INTO panier_produit (id_panier, id_produit, quantite) VALUES (:panierId, :produitId, :quantite)';
        $conn->executeStatement($sql, [
            'panierId' => $panier->getId(),
            'produitId' => $produit->getId(),
            'quantite' => $pendingProduct['quantite']
        ]);
        
        // Update cart total price
        $newTotal = $produit->getPrix() * $pendingProduct['quantite'];
        $panier->setPrixTotal($newTotal);
